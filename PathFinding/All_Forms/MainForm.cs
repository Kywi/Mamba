...
namespace PathFinding
{
    public partial class Form1 : Form
    {
        ...
        private List<Action> upload_actions = new List<Action>();
        private List<Action> save_actions = new List<Action>();
        ...
        private void basic_sizes_AND_settings()
        {
            ...
            upload_actions.Add(Load_From_Image);
            upload_actions.Add(Read_from_File);

            save_actions.Add(write_to_file);
            save_actions.Add(save_Img);

            (new ToolTip()).SetToolTip(button9,"Перейти до наступного кроку лабіринту");
            (new ToolTip()).SetToolTip(button4, "Очистка пройденого шляху");
            (new ToolTip()).SetToolTip(comboBox1,"Оберіть алгоритм проходження лабіринту");
            (new ToolTip()).SetToolTip(comboBox2, "Оберіть алгоритм генерації лабіринту");
            (new ToolTip()).SetToolTip(button6, "Перегляд документації");
            (new ToolTip()).SetToolTip(size_cell, "Розмірність 1 комірки в пікселях");
            (new ToolTip()).SetToolTip(cColunm_X, "Кількість колонок");
            (new ToolTip()).SetToolTip(cRows_Y, "Кількість рядків");
            (new ToolTip()).SetToolTip(button3, "Зберегти лабіринт");
            (new ToolTip()).SetToolTip(button2, "Завантажити лабіринт");
            (new ToolTip()).SetToolTip(checkBox2, "Алгоритми проходження отримують можливість будувати шлях по діагоналі");
            (new ToolTip()).SetToolTip(checkBox1, "Алгоритми по досягненню фінішної точки зупиняють виконання і будують шлях");
            (new ToolTip()).SetToolTip(trackBar1, "Регулює швидкість демонстраційного режиму роботи алгоритму");
            (new ToolTip()).SetToolTip(radioButton1, "Відображається тільки результат виконання алгоритму");
            (new ToolTip()).SetToolTip(radioButton3, @"Показується як алгоритм працює у режимі ""Кіно""");
            (new ToolTip()).SetToolTip(radioButton2, "Алгоритм переходить до наступного кроку тільки після натиснення кнопки нижче");
        ...
        public void save_Img()
        {
            ...
            {
                if (saveFileDialog2.FileName == null) return;
                Draw.Anim.getBlack_WhiteImage().Save(saveFileDialog2.FileName);
        ...
        {
            SaveForm saveForm = new SaveForm(save_actions);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            Upload_Form upload = new Upload_Form(upload_actions);
            upload.ShowDialog();
        }
        //----------------------------------------------------------------------------------
